...
import { admissionErrorText, admissionTagType } from './admissionRules.ts'
...
<template>
  <div class="mine-layout admission-page pt-3">
    <el-card shadow="never">
      <template #header><div class="page-header"><span>Trial Lessons</span><el-button type="primary" @click="submitTrial">Schedule Trial</el-button></div></template>
      ...
      <el-form :inline="true" :model="search" class="search-form">
        <el-form-item label="Date"><el-input v-model="search.date" placeholder="YYYY-MM-DD" /></el-form-item>
        <el-form-item label="Lead"><el-input-number v-model="form.lead_id" :controls="false" /></el-form-item>
        <el-form-item label="Student"><el-input-number v-model="form.lead_student_id" :controls="false" /></el-form-item>
        <el-form-item label="Teacher"><el-input-number v-model="form.teacher_id" :controls="false" /></el-form-item>
        <el-form-item><el-button @click="loadRows">Search</el-button></el-form-item>
      </el-form>
      <el-table v-loading="loading" :data="rows" row-key="id">
        <el-table-column prop="lead_id" label="Lead" width="100" />
        <el-table-column prop="teacher_id" label="Teacher" width="120" />
        <el-table-column prop="start_time" label="Start" width="180" />
        <el-table-column prop="end_time" label="End" width="180" />
        ...
        <el-table-column label="Actions" fixed="right" width="140"><template #default="{ row }"><el-button link type="primary" @click="markAttended(row)">Attend</el-button></template></el-table-column>
        <template #empty><el-empty description="No trial lessons" /></template>
      </el-table>
