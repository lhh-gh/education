...
async function loadRows() {
  ...
  try {
    ...
    errorText.value = error?.message ?? 'Attendance report loading failed'
  }
  finally {
    loading.value = false
  }
...
function handleReset() {
  Object.assign(search, { page: 1, pageSize: 20, campus_id: undefined, class_id: undefined, teacher_id: undefined, course_id: undefined, attendance_status: undefined, group_by: undefined, ...quickReportRange('this_month') })
  loadRows()
}
...
<template>
  <div class="mine-layout education-report-page pt-3">
    <el-card shadow="never">
      <template #header>Attendance Report</template>
      ...
      <template v-else>
        ...
        <ReportTableToolbar title="Attendance Rows" :total="total" :loading="loading" @refresh="loadRows" />
        <el-table :data="rows" row-key="lesson_id">
          <el-table-column prop="date" label="Date" width="130" />
          <el-table-column prop="class_name" label="Class" min-width="150" />
          <el-table-column prop="teacher_name" label="Teacher" min-width="140" />
          <el-table-column prop="course_name" label="Course" min-width="140" />
          <el-table-column prop="lesson_title" label="Lesson" min-width="180" />
          <el-table-column prop="student_name" label="Student" min-width="140" />
          <el-table-column label="Status" width="110">
            <template #default="{ row }"><el-tag :type="reportTagType(row.attendance_status)">{{ row.attendance_status }}</el-tag></template>
          </el-table-column>
          <el-table-column prop="consume_policy" label="Policy" width="120" />
          <el-table-column prop="consumed_units" label="Units" width="100" />
          <el-table-column prop="submitted_at" label="Submitted At" width="180" />
          <el-table-column label="Actions" width="110" fixed="right">
            <template #default="{ row }">
              <router-link v-if="canShowReportDrillLink(canDrill, row.lesson_id)" :to="`/education/academic/attendance-review?lesson_id=${row.lesson_id}`">Detail</router-link>
            </template>
          </el-table-column>
        </el-table>
