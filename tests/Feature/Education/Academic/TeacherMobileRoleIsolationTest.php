 ...
 * @coversNothing
 */
final class TeacherMobileRoleIsolationTest extends ProfileRecordAdminCase
{
    ...
    public function testGuardianRoleReturns403OnLeaveEndpoints(): void
    {
        ...
        $reject = $this->put('/mobile/education/academic/teacher/leave-requests/' . $fixture['leave']->id . '/reject', [
            'review_remark' => 'Rejected',
        ], $this->mobileHeaders($fixture['tenant']));

        foreach ([$page, $detail, $approve, $reject] as $result) {
            self::assertSame(ResultCode::FORBIDDEN->value, $result['code']);
            self::assertSame('teacher mobile role required', $result['message']);
        }
    }

    private function fixture(): array
    {
        $tenant = $this->tenant('teacher_mobile_role_isolation');
        $campus = $this->campus($tenant, 'main');
        $profile = EducationUserProfile::query()->create(['profile_key' => 'tenant:' . $tenant->id . ':' . $this->user->id, 'tenant_id' => $tenant->id, 'user_id' => $this->user->id, 'role_code' => 'guardian', 'display_name' => 'Guardian Mobile', 'status' => 'enabled', 'current_campus_id' => $campus->id]);
        ...
        $leave = EducationLeaveRequest::query()->create(['tenant_id' => $tenant->id, 'campus_id' => $campus->id, 'leave_no' => 'LEA-ROLE', 'source' => 'staff', 'leave_type' => 'sick', 'lesson_id' => $lesson->id, 'lesson_student_id' => $lessonStudent->id, 'class_id' => $class->id, 'course_id' => $course->id, 'student_id' => $student->id, 'account_id' => $account->id, 'teacher_id' => 9901, 'reason' => 'Sick leave', 'status' => 'pending', 'makeup_required' => true]);

        return ['tenant' => $tenant, 'campus_id' => (int) $campus->id, 'leave' => $leave];
    }
