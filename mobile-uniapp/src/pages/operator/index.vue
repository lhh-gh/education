...
import { getOperatorContext } from '@/api/foundation/context'
import { operatorPageOptions } from '@/pages/foundation/pageOptions'
...
const {
  ...
  retry,
  refresh,
} = createFoundationContextPage(() => getOperatorContext(), operatorPageOptions)
  ...
  <view class="page">
    ...
    <view v-else-if="state.context" class="content">
      <view class="summary">
        ...
        <text class="meta">Role: {{ state.context.profile.role_code }}</text>
        <text class="meta">{{ currentCampusName || 'Tenant wide' }}</text>
      </view>
