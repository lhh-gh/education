...
import type { TeacherTrialLesson } from '@/api/admissions/teacher'
import { getTeacherTrialLessons } from '@/api/admissions/teacher'
...
async function loadRows(): Promise<void> {
  ...
  try {
    const result = await getTeacherTrialLessons({ date: state.date })
    state.lessons = result.list
    state.status = state.lessons.length === 0 ? 'empty' : 'success'
    state.message = state.lessons.length === 0 ? 'No trial lessons' : ''
  }
  catch (error) {
    state.lessons = []
...
function openFeedback(lesson: TeacherTrialLesson): void {
  uni.navigateTo({ url: `/pages/teacher/admissions/trial-feedback?trial_lesson_id=${lesson.id}` })
}
...
<template>
  <view class="page">
    ...
    <TeacherStateBlock v-if="state.status === 'loading'" status="loading" message="Loading trial lessons" />
