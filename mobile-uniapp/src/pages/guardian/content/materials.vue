...
import { getGuardianContentMaterials } from '@/api/content/guardian'
...
async function loadRows(): Promise<void> {
  ...
  try {
    const result = await getGuardianContentMaterials({ student_id: state.studentId })
...
function openDetail(row: GuardianMaterial): void {
  uni.navigateTo({ url: `/pages/guardian/content/material-detail?student_id=${state.studentId}&material_id=${row.material_id || row.id}` })
}
...
<template>
  <view class="page">
    <GuardianStateBlock v-if="state.status === 'loading'" state="loading" message="Loading materials" />
    ...
    <view v-else class="list">
      <view v-for="row in state.rows" :key="row.material_id || row.id" class="card" @tap="openDetail(row)">
        <text class="title">{{ row.material_name || `Material ${row.material_id || row.id}` }}</text>
        <text class="meta">{{ row.material_type || 'material' }}</text>
      </view>
    </view>
  </view>
</template>
